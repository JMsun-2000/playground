...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 29 21:48:21 2021
...
and frequencies w1=.05,w2=.4,w3=.08,w4=.04,w5=.1,w6=.1,w7=.23. What is the minimum-possible average search time 
of a binary search tree with these keys?
'''
        ...
        for i in np.arange(n-s):
            ...
            for j in np.arange(s+1):
                ...
                if i+s<n and r+1<= i+s:
                    temp[j] += m[r+1,i+s]
                print('r=',r, ' temp=', temp[j])
            m[i,i+s]=min(temp)
            print('min_temp=', m[i,i+s])
    return m[0,-1]
